...
const handler = async (m, { conn, text, usedPrefix, command }) => {
    try {
        ...
        if (['play', 'yta', 'ytmp3', 'playaudio'].includes(command)) {
            ...
            if (!audio?.url) throw '⚠ No se pudo obtener el audio. Revisa si la URL es válida o intenta otro video.'
            ...
            if (!video?.url) throw '⚠ No se pudo obtener el video. Revisa si la URL es válida o intenta otro video.'
...
async function fetchFromApis(apis) {
    for (const { api, endpoint, extractor } of apis) {
        try {
            ...
            console.warn(`⚠ Error en API ${api}:`, e.message || e)
        }
        await new Promise(resolve => setTimeout(resolve, 500))
    }
    return { url: null, api: null }
}
