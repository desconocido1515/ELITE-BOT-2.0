...
async function formatMessageStubType(stubType) {
  const map = {
    2: 'Grupo eliminado',
...
export default async function printMessage(m, conn = { user: {} }) {
  ...
  // Detectar si es el bot principal o un subbot
  let botName
  if (m.sender === conn.user.jid) {
    botName = 'Bot Principal'
  } else {
    let subBotName = await conn.getName(m.sender)
    botName = `Subbot ~ ${subBotName}`
  }
...
┃ ❖ Acción: ${m.messageStubType ? await formatMessageStubType(m.messageStubType) : (m.text || 'Mensaje normal')}
