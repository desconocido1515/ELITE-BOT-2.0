    ...
    conn.sendMessage(jid, { video: gif, gifPlayback: true, gifAttribution: b, caption: text, footer: footer, jpegThumbnail: file, templateButtons: but, ...options })
        ...
        cMod: {
            ...
            enumerable: true
                ...
                copyNForward: {
            /**
             * Exact Copy Forward - VERSIÓN MEJORADA
             * @param {String} jid
             * @param {import('@adiwajshing/baileys').proto.WebMessageInfo} message
            ...
            async value(jid, message, forwardingScore = true, options = {}) {
                try {
                    ...
                    // Filtrar tipos de mensaje no reenviables
                    const nonForwardableTypes = [
                        'protocolMessage', 
                        'messageContextInfo',
                    ...
                    if (nonForwardableTypes.includes(mtype)) {
                        throw new Error(`No se puede reenviar mensajes de tipo: ${mtype}`)
                    }
                    
                    // Verificar si es mensaje de canal/estado
                    if (message.key?.remoteJid === 'status@broadcast') {
                        throw new Error('No se pueden reenviar mensajes de estados/canales')
                    }
                    
                    let m = generateForwardMessageContent(message, !!forwardingScore)
                    
                    if (!m || Object.keys(m).length === 0) {
                        throw new Error('No se pudo generar contenido para reenviar')
                    }
                    
                    let ctype = Object.keys(m)[0]
                    
                    // Manejar forwarding score
                    if (forwardingScore && typeof forwardingScore === 'number' && forwardingScore > 1) {
                        if (m[ctype].contextInfo) {
                            m[ctype].contextInfo.forwardingScore = (m[ctype].contextInfo.forwardingScore || 0) + forwardingScore
                        }
                    }
                    
                    // Combinar contextInfo
                    m[ctype].contextInfo = {
                    ...
                    let waMessage = generateWAMessageFromContent(jid, m, {
                        ...options,
                        userJid: conn.user.jid
                    })
                    ...
                    console.error(chalk.red('❌ Error en copyNForward:'), error.message)
                    // En lugar de lanzar error, enviar mensaje informativo
                    await conn.sendMessage(jid, { 
                        text: `⚠️ No se pudo reenviar el mensaje: ${error.message}` 
                    })
                    return null
                }
            },
