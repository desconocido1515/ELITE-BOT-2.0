...
export default async function printMessage(m, conn = { user: {} }) {
    ...
    const botTag = conn.user?.jid !== global.conn.user?.jid ? ' 【𝗚𝗕 - 𝗦𝗨𝗕 𝗕𝗢𝗧】' : ''
...
┃ ❖ ${chalk.white.bold('Hora:')} ${new Date((m.messageTimestamp?.low || m.messageTimestamp) * 1000).toTimeString()}
┃ ❖ ${chalk.white.bold('Acción:')} ${WAMessageStubType[m.messageStubType] || m.mtype}
